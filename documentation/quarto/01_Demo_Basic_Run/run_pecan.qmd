...
```{r}
# Generate workflow ID  (e.g., 20250619154701)
workflow_id <- format(Sys.time(), "%Y%m%d%H%M%S")

# Replace all occurrences of "ID" in the XML file with that workflow_id
system(glue::glue("sed -i 's|ID|{workflow_id}|g' {settings_path}"))
```
